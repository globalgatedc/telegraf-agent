...
apt)
	...
	DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a apt-get update && apt-get install --no-install-recommends --no-upgrade -y telegraf
	;;
yum)
...
case $option in
1)
	configURL="https://raw.githubusercontent.com/globalgatedc/telegraf-agent/main/telegraf_linux.conf"
	;;
*)
